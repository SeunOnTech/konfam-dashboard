// components/monitoring/all-threats-modal.tsx
"use client"
...
export function AllThreatsModal({
  threats,
  isOpen,
  onClose,
  onSelectThreat,
  selectedId,
}: AllThreatsModalProps) {
  ...
  const filteredAndSortedThreats = useMemo(() => {
    ...
    // Filter by search query against content, author, keywords (if any)
    // Filter by search query against content, author, or summary
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase()
      result = result.filter((t) => {
        const summary = t.verificationSummary?.toLowerCase() || ""
        const threatType = t.threatType?.toLowerCase() || ""
        const severity = t.severity?.toLowerCase() || ""
        const postText = t.detectedPost?.content?.toLowerCase() || ""
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case "panic": {
          const aPanic = Math.abs(a.sentimentImpact || 0)
          const bPanic = Math.abs(b.sentimentImpact || 0)
          return bPanic - aPanic
        }
        case "engagement": {
          // Prefer detectedPost’s engagement if available; fallback to currentEngagement
          const aLikes = a.detectedPost?.likeCount || 0
          const aRts = a.detectedPost?.retweetCount || 0
          const aReplies = a.detectedPost?.replyCount || 0
          const aEngagement = aLikes + aRts + aReplies + (a.currentEngagement || 0)

          const bLikes = b.detectedPost?.likeCount || 0
          const bRts = b.detectedPost?.retweetCount || 0
          const bReplies = b.detectedPost?.replyCount || 0
          const bEngagement = bLikes + bRts + bReplies + (b.currentEngagement || 0)

          return bEngagement - aEngagement
        }
        case "recent": {
          const aTime = new Date(a.detectedAt || (a as any).capturedAt || Date.now()).getTime()
          const bTime = new Date(b.detectedAt || (b as any).capturedAt || Date.now()).getTime()
          return bTime - aTime
        }
        default:
          return 0
              ...
              {showSortMenu && (
                <div className="absolute right-0 mt-2 w-48 bg-card border border-border rounded-lg shadow-lg z-10 animate-in fade-in slide-in-from-top-2 duration-200">
                  {[
                    ...
                    { value: "panic" as const, label: "By Sentiment Impact" },
